...
export const SupercreateClan = async (req, res) => {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    const { name, address, phonenumber, creatorEmail } = req.body;
    ...
    if (!name || !creatorEmail) {
      ...
      return res.status(400).json({
        success: false,
        message: "Clan name and creator email are required",
        missingFields: {
          name: !name,
    ...
    if (!creator) {
      ...
      creator = new User({
        email: creatorEmail.toLowerCase().trim(),
        password: hashedPassword, //"123456789", //generateTemporaryPassword(), // Implement this
        isVerified: true,
        roles: ["user"],
      });
    ...
    // Generate clan ID
    const preliminaryClanID = generateClanID(name);

    // Check for existing clan
    const existingClan = await Clan.findOne({
      $or: [
        { name },
        { email: creator.email }, // Now using creator's email
        { uniqueClanID: preliminaryClanID },
      ],
    }).session(session);
    ...
    if (existingClan) {
      await session.abortTransaction();
      const conflictField =
        existingClan.name === name
          ? "name"
          : existingClan.email === creator.email
          ? "email"
          : "uniqueClanID";

      return res.status(409).json({
        success: false,
        message: `Clan ${conflictField} already exists`,
        conflictField,
        existingClan: {
          id: existingClan._id,
    ...
    // Create clan using creator's email
    const newClan = new Clan({
      name,
      email: creator.email, // Using creator's email
      address,
      phonenumber,
      creator: creator._id,
      uniqueClanID: preliminaryClanID,
      ...
      members: [
        {
          ...
          status: "approved",
          joinedAt: new Date(),
          memberCode: generateFirstMemberCode(preliminaryClanID),
          homeAddress: address || "",
          phonenumber: phonenumber || "",
        },
      ],
    ...
    // Create wallet
    const clanWallet = new ClanWallet({
      clan: savedClan._id,
      balance: 0,
      currency: "NGN",
    });
    ...
    // Update user's clan references
    creator.clans = creator.clans || [];
    creator.clans.push(savedClan._id);
    await creator.save({ session });
    ...
    return res.status(201).json({
      ...
      data: {
        clan: {
          _id: savedClan._id,
          name: savedClan.name,
          uniqueClanID: savedClan.uniqueClanID,
          creator: {
            _id: creator._id,
            email: creator.email,
            memberCode: savedClan.members[0].memberCode,
          },
        },
        wallet: {
          balance: clanWallet.balance,
          currency: clanWallet.currency,
        },
      },
    });
  } catch (error) {
    ...
    if (error.code === 11000) {
      ...
      return res.status(409).json({
        ...
        solution:
          field === "email"
            ? "The creator email is already associated with another clan"
...
function generateClanID(name) {
  ...
  const initials = name
    ...
    .join("");
  return `${initials}-${monthYear}`;
}

function generateFirstMemberCode(clanID) {
  // Extract the prefix part (before date) from clanID
  const clanPrefix = clanID.split("-")[0];
  return `${clanPrefix}-AAAA-0001`;
}

// For adding subsequent members (not used in creation but included for completeness)
export async function generateNextMemberCode(clanId) {
  const clan = await Clan.findById(clanId);
  if (!clan) throw new Error("Clan not found");

  const lastMember = clan.members.sort((a, b) => b.joinedAt - a.joinedAt)[0];

  const lastCode = lastMember?.memberCode;
  return generateUniqueMemberID(clan.uniqueClanID, lastCode);
}

// Your sequence generators (unchanged)
const getNextNumericSequence = (current) => {
  if (!current) return "0001";
  const num = parseInt(current) + 1;
  return num > 9999 ? "0001" : String(num).padStart(4, "0");
...
export const getClanWithHouseholdsAndWallet = async (clanId) => {
  try {
    const result = await Clan.aggregate([
      // Match the specific clan - using new ObjectId syntax
      { $match: { _id: new mongoose.Types.ObjectId(clanId) } },

      // Lookup households belonging to this clan
      {
        $lookup: {
          from: "households",
          localField: "_id",
      ...
      // Unwind the wallet array (since it's one-to-one)
      ...
      // Add computed fields
      {
        $addFields: {
          ...
          totalHouseholds: { $size: "$households" },
      ...
      // Project the final structure
      {
        $project: {
          ...
          members: 1,
          households: 1,
          settings: 1, // 👈 Add this line
          availableApartmentTypes: 1, // 👈 Add this lin
          availableStreets: 1, // 👈 Add this line
          wallet: {
    ...
    // Populate user references in the result
    const populatedResult = await Clan.populate(result[0], [
      { path: "creator", select: "name email" },
      { path: "admins.user", select: "name email" },
    ...
    console.error("Error fetching clan with households and wallet:", error);
    throw error;
  }
};
// In your controller
export const getClanDetails = async (req, res) => {
  try {
    ...
    const clanData = await getClanWithHouseholdsAndWallet(clanId);
...
export const updateClanSettings = async (req, res) => {
  const newSettings = req.body; // Expects { clanId: "...", allowUsersToManageRequests: true, ... }

  try {
    // 1. Validate clanId exists in the request body
    if (!newSettings.clanId) {
      return res
        .status(400)
        .json({ message: "Clan ID is required in the request body." });
    }

    // 2. Find the clan using clanId from the request body
    const clan = await Clan.findById(newSettings.clanId);

    if (!clan) {
      return res.status(404).json({ message: "Clan not found." });
    }

    // 3. Remove clanId from settings (we don't want to save it in settings)
    const { clanId, ...settingsToUpdate } = newSettings;
    ...
    if (settingsToUpdate && Object.keys(settingsToUpdate).length > 0) {
      ...
      // Update only valid settings that exist in the schema
      for (const [key, value] of Object.entries(settingsToUpdate)) {
        if (key in clan.settings) {
          clan.settings[key] = value;
        }
      }

      // Mark settings as modified
      clan.markModified("settings");

      // 5. Save the updated clan
      await clan.save();
      ...
      return res
        .status(400)
        .json({ message: "No valid settings provided for update." });
    }
  } catch (error) {
...
export const updateClanSettings_str_aprtmenttype = async (req, res) => {
  try {
    const { clanId, streets, apartmentTypes } = req.body;

    const updateFields = {};
    let hasUpdates = false;

    // Add streets if provided and valid (array of strings)
    if (Array.isArray(streets) && streets.length > 0) {
      updateFields.$addToSet = { availableStreets: { $each: streets } };
      hasUpdates = true;
    }

    // Add apartment types if provided and valid (array of strings)
    if (Array.isArray(apartmentTypes) && apartmentTypes.length > 0) {
      // If $addToSet already exists for streets, merge the apartmentTypes into it
      if (updateFields.$addToSet) {
        updateFields.$addToSet.availableApartmentTypes = {
          $each: apartmentTypes,
        };
      } else {
        updateFields.$addToSet = {
          availableApartmentTypes: { $each: apartmentTypes },
        };
      }
      hasUpdates = true;
    }
    ...
    if (!hasUpdates) {
      return res.status(400).json({
        message: "No streets or apartment types provided for update.",
      });
    }

    // Find and update the clan document
    ...
    res.status(200).json({
      message: "Clan options updated successfully.",
      clan: {
        _id: clan._id,
        name: clan.name,
        availableStreets: clan.availableStreets,
        availableApartmentTypes: clan.availableApartmentTypes,
      },
    });
  } catch (error) {
    console.error("Error updating clan settings:", error);
...
export const createEstateWithDevice = async (req, res) => {
  try {
    const { clanId, estateCode } = req.body;

    // Validate clanId is a valid ObjectId
    ...
    const existingRegistration = await PhysicalDeviceClan.findOne({
      $or: [{ clan: clanId }, { estateCode }],

      if(existingRegistration) {
        return res.status(400).json({
          success: false,
          message: existingRegistration.clan.equals(clanId)
            ? "Clan already has a physical device registered"
            : "Estate code already in use",
        });
      },
    });
    const newDevice = new PhysicalDeviceClan({
      clan: clanId,
      estateCode,
    });
    ...
    res.status(201).json({
      success: true,
      data: newDevice,
      message: "Estate with physical device created successfully",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const getAllPhysicalDeviceClans = async (req, res) => {
  try {
    const { page = 1, limit = 10 } = req.query;

    const devices = await PhysicalDeviceClan.find()
      // .limit(limit * 1)
      // .skip((page - 1) * limit)
      .populate("clan", "name email uniqueClanID");

    const count = await PhysicalDeviceClan.countDocuments();
    ...
    res.status(200).json({
      ...
      data: devices,
      message: "All physical device clans retrieved successfully",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const TestThis = async (req, res) => {
  const { estateId, code } = req.body;

  // Basic validation
  if (!estateId || !code) {
    return res.status(400).json({
      success: false,
      message: "Estate ID and code are required",
    });
  }

  // Call the service function
  const result = await sendGPassCode(estateId, code);
...
export const kakaupdateClanSettings = async (req, res) => {
  // 2. Find the clan using clanId from the request body
  const clan = await Clan.find();

  return res
    .status(400)
    .json({ message: "No valid settings provided for update.", clan });
};
